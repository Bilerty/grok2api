  ...
  "zh-CN": {
    translation: {
      ...
      statusReauthRequired: "失效",
        ...
        consoleBotRisk: "Console 风控探测",
        consoleBotRiskTooltipSafe: "Console 风控探测：安全（botFlagSource=0）· 探测时间 {{time}}",
        consoleBotRiskTooltipSource1: "Console 风控探测：存在风控（botFlagSource=1）· 探测时间 {{time}}",
        consoleBotRiskTooltipSource2: "Console 风控探测：存在风控（botFlagSource=2）· 探测时间 {{time}}",
      ...
      settings: {
        ...
        accounts: {
          ...
          excludeConsoleBotFlaggedFromSchedulingHelp: "开启后，Console 上游检测到风控（botFlagSource 为 1 或 2）的账号不再参与调度，并同步移出其链接的 Web / Build 同账号。该开关与「风控 Build 号移出调度」相互独立。账号仍保留在池中，可手动查看与操作。",
          consoleDailyResetScheduling: "开启24h额度刷新标记（随机时间）",
          consoleDailyResetSchedulingHelp: "开启后，每日 UTC 0:00 将全部 Console 账号的额度刷新（含风控探测）随机排布在未来 24h 内，避免集中刷新；启动时若开关已开启且当天未排班，也会立即排一次。仅在「风控 Console 号移出调度」开启时可操作。",
          consoleScheduleNow: "立即排班未来24h额度刷新",
          consoleScheduledCount: "已排班 {{count}} 个 Console 账号",
          consoleScheduleFailed: "排班失败，请稍后重试",
          cleanupTitle: "账号清理",
